 ...
 BEGIN
	...
	DECLARE ici_utmp INT;
	 ...
	 declare cltt cursor for
        select  PollID, TopicID, UserID from {databaseName}.{objectQualifier}Topic where PollID IS NOT NULL;  		
...
IF NOT EXISTS (SELECT 1 FROM information_schema.COLUMNS 
 ...
 IF NOT EXISTS (SELECT 1 FROM information_schema.COLUMNS 
  ...
  AND COLUMN_NAME='Culture' LIMIT 1) THEN
        ALTER TABLE  {databaseName}.{objectQualifier}User ADD `Culture` CHAR(5) NULL;
  END IF;
